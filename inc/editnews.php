	...
	if($newsId)
    {
		...
		if(mysqli_num_rows($result))
        {
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
            {
                ...
                {
                    $trust .= '<li>Не указан заголовок статьи: длина заголовка должна быть не менее 8-ми символов</li>';
                }
				if(mb_strlen($shortDescription) < 30)
                {
                    $trust .= '<li>Не указано краткое описание новости: длина не мнее 30 символов</li>';
                }
				if(mb_strlen($fullDescription) < 30) 
                {
                    $trust .= '<li>Не указано полное описание новости: длина не мнее 30 символов</li>';
                }
				...
				if(!$trust)
                {
					...
					if(mysqli_query($db, $SQL))
                    {
                        ...
                        {
							out_mod("<span style='display:block; margin-left:30px;'>Данные сохранены. <a href='/?do=admin&section=allnews' style='text-decoration:underline;'>К списку всех новостей</a></span>");
						}
                        else
                        {
